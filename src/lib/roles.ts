...
export function linkInvitacionColaborador(empresaId: string) {
  const path = `/registro?empresa=${encodeURIComponent(empresaId)}`
...
export function textoLinkInvitacion(empresaId: string) {
  return `analitica360.app/registro?empresa=${empresaId}`
}
