...
import { anularCompra, formatoFechaCompra, type CompraFila } from '../lib/compras'
...
export function AnularCompraModal({
  ...
  async function confirmar() {
    ...
    setEnviando(true)
    const fallo = await anularCompra(requireSupabase(), compra.id, motivo.trim())
    setEnviando(false)
    if (fallo) {
      setError(fallo)
      return
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-[440px] rounded-lg bg-white/95 p-8 shadow-[0_20px_60px_rgba(0,0,0,0.3)]">
        <h2 className="text-xl font-bold text-[#1A2F4A]">¿Confirmar anulación de esta compra?</h2>
        <div className="mt-4 rounded-md bg-[#EEF2F6] px-3 py-3 text-sm text-[#1A2F4A]">
          <p>Fecha: {formatoFechaCompra(compra.fecha)}</p>
          <p className="mt-1">Proveedor: {compra.proveedor || '—'}</p>
          <p className="mt-1">Productos: {compra.productos || '—'}</p>
          <p className="mt-1 font-semibold">Total: {formatoARS(compra.total)}</p>
        </div>
