DROP FUNCTION IF EXISTS public.registrar_empresa(text, text, text);
...
CREATE OR REPLACE FUNCTION public.admin_listar_planes()
RETURNS TABLE (
  id uuid,
  nombre text
)
...
BEGIN
  ...
  FROM public.planes p
  WHERE p.activo IS DISTINCT FROM (1 = 0)
  ORDER BY p.precio_ars, p.nombre;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.admin_listar_planes() TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.admin_asignar_suscripcion(uuid, uuid, date) TO authenticated;
